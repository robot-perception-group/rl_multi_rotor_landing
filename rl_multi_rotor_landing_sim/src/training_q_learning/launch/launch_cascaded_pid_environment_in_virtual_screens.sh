...
screen -d -m -S "$(echo $SIM_ID)_cascaded_pid_interface" bash -i -c "$(echo $ROS_PROJECT_ROOT)/src/training_q_learning/launch/launch_cascaded_pid_interface.sh $MAV_NAME $ROS_PORT $GAZ_PORT" &
echo "Launched cascaded PID controller interface."
