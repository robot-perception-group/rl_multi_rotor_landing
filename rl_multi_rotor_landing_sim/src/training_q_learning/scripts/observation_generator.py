...
action_to_interface_topic = ('training_action_interface/action_to_interface',ActionMsg)

#Topic definition of publishers
observation_topic = ('training_observation_interface/observations',ObservationRelativeStateMsg)
observation_action_topic = ('training_observation_interface/observations_actions',ActionMsg)
...
class ObservationRelativeState():
    ...
    def check_frames(self):
        ...
        frame_ids = [self.relative_position.header.frame_id,self.relative_velocity.header.frame_id,self.relative_acceleration.header.frame_id]
...
#Define functions
def read_simulation_reset(msg):
    """Function sets a flag when the simulation is reset."""
    observation_relative_state.request_simulation_reset = True
    return
...
def read_relative_position(msg):
    ...
    ObservationRelativeState and adds Gaussian zero mean noise.
    '''
...
def read_relative_velocity(msg):
    ...
    #Reset the low-pass filter when simulation is reset is requested
    if observation_relative_state.request_simulation_reset:
        observation_relative_state.relative_acceleration.linear_acceleration.x = 0
        observation_relative_state.relative_acceleration.linear_acceleration.y = 0
        observation_relative_state.relative_acceleration.linear_acceleration.z = 0
        rel_v_x = 0
        rel_v_y = 0
        rel_v_z = 0
        observation_relative_state.request_simulation_reset = False
...
if __name__ == '__main__':
    ...
    reset_simulation_subscriber = rospy.Subscriber(reset_simulation_topic[0],reset_simulation_topic[1],read_simulation_reset)
