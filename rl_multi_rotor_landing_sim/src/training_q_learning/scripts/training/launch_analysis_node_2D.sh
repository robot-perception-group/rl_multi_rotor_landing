...
echo $ROS_IP
echo $ROS_HOSTNAME
echo "launch_training_action_interface_2D.sh: ROS_MASTER_URI=$ROS_MASTER_URI"
echo "launch_training_action_interface_2D.sh: GAZEBO_MASTER_URI=$GAZEBO_MASTER_URI"

#Source workspace
source $ROS_PROJECT_ROOT/other_files/setup.bash
source $ROS_PROJECT_ROOT/other_files/activate_training_venv.sh

roslaunch training_q_learning analysis_node_2D.launch drone_name:=$DRONE_NAME ros_port:=$ROS_PORT gaz_port:=$GAZ_PORT ros_ip:=$ROS_IP