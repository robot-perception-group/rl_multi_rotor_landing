...
the drone and the moving platform in stability axes and the world frame.
    ...
    - Moving platform: drone_name/landing_simulation/moving_platform/state
                       drone_name/landing_simulation/world_frame/moving_platform/state
...
def read_gazebo_model_states(msg):
    ...
    """
    #Extract data
    #Data is by default delivered in world frame
    model_names = getattr(msg,"name")
    pose_list = getattr(msg,"pose")
    twist_list = getattr(msg,"twist")

    #Get ids of objects in message
    drone_id = model_names.index(drone_name)
    mp_id = model_names.index("moving_platform")

    #Store data
    moving_platform_state_original.pose.header.frame_id = gazebo_model_state_frame
    moving_platform_state_original.pose.pose = pose_list[mp_id]

    moving_platform_state_original.twist.header.frame_id = gazebo_model_state_frame
    moving_platform_state_original.twist.twist.linear.vector = twist_list[mp_id].linear
    moving_platform_state_original.twist.twist.angular.vector = twist_list[mp_id].angular

    drone_state_original.pose.header.frame_id = gazebo_model_state_frame
    drone_state_original.pose.pose = pose_list[drone_id]

    drone_state_original.twist.header.frame_id = gazebo_model_state_frame
    drone_state_original.twist.twist.linear.vector = twist_list[drone_id].linear
    drone_state_original.twist.twist.angular.vector = twist_list[drone_id].angular  
    return   
