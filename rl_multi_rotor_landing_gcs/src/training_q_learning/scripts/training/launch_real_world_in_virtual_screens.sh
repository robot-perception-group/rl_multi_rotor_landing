#Script launches real world in virtual screens
...
screen -d -m -S "$(echo $SIM_ID)_hitl_interface" bash -i -c "$(echo $ROS_PROJECT_ROOT)/src/training_q_learning/scripts/training/launch_hitl_interface.sh $MAV_NAME $ROS_PORT $GAZ_PORT" &
echo "Launched hitl interface."
